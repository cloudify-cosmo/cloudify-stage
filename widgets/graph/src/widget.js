...
Stage.defineWidget({
    ...
    initialConfiguration: [
        Stage.GenericConfig.POLLING_TIME_CONFIG(5),
        {
            id: 'nodeFilter',
            name: 'Node filter',
            description: 'Node filter to limit number of available metrics',
            type: Stage.Basic.GenericField.CUSTOM_TYPE,
            component: Stage.Basic.NodeFilter,
            default: Stage.Basic.NodeFilter.EMPTY_VALUE,
            storeValueInContext: true
        },
        {
            id: 'charts',
            name: 'Charts table',
            description: 'Charts configuration table',
            default: '',
            type: Stage.Basic.GenericField.CUSTOM_TYPE,
            component: Stage.Basic.Form.Table,
            rows: 5,
            columns: [
                {
                    name: 'metric',
                    ...
                    default: '',
                    type: Stage.Basic.GenericField.CUSTOM_TYPE,
                    component: Stage.Basic.MetricFilter,
                    description: 'Metric to be presented on the chart',
        ...
        {
            id: 'query',
            name: 'Custom Influx Query',
            description: "Please note that below query builder overrides the series defined in 'Charts table'",
            default: '',
            type: Stage.Basic.GenericField.CUSTOM_TYPE,
            component: Stage.Basic.Form.Table,
            rows: 1,
            columns: [
                ...
                },
                {
                    ...
                    type: Stage.Basic.GenericField.STRING_TYPE,
                    description:
                        "You can use ${deploymentId} token to inject dynamic deployment ID. Example: '/${deploymentId}..*.((memory_MemFree))$/'"
                },
                {
                    name: 'qWhere',
                    label: 'WHERE',
                    default: '',
                    type: Stage.Basic.GenericField.STRING_TYPE,
                    description: 'You can use ${timeFilter} token to inject dynamic data/time ranges.'
                }
            ]
        },
        {
            ...
            default: Stage.Basic.Graphs.Graph.LINE_CHART_TYPE,
            type: Stage.Basic.GenericField.LIST_TYPE
        },
        {
            id: 'timeFilter',
            name: 'Time range and resolution',
            description: 'Time range and time resolution for all defined charts',
            type: Stage.Basic.GenericField.CUSTOM_TYPE,
            component: Stage.Basic.TimeFilter,
            default: Stage.Basic.TimeFilter.INFLUX_DEFAULT_VALUE,
            defaultValue: Stage.Basic.TimeFilter.INFLUX_DEFAULT_VALUE
        }
    ],
    UNCONFIGURED_STATE: 'unconfigured',
    EMPTY_RESPONSE_STATE: 'emptyResponse',

    _prepareData(data, xDataKey) {
        ...
        for (let i = 0; i < NUMBER_OF_POINTS; i++) {
            const point = {
                [xDataKey]: Stage.Utils.Time.formatTimestamp(
                    data[REFERENCE_METRIC_INDEX].points[i][TIME_INDEX],
                    TIME_FORMAT,
                    null
                )
            };
            for (let j = 0; j < NUMBER_OF_METRICS; j++) {
                if (
                    ...
                    const metricName = data[j].name;
                    const pointValue = data[j].points[i][VALUE_INDEX];
        ...
        if (points.length > MAX_NUMBER_OF_POINTS) {
            ...
            const delta = parseFloat(points.length / MAX_NUMBER_OF_POINTS);
            for (let i = 0; i < points.length; i += delta) {
    ...
    _getChartsConfiguration(charts, query, data) {
        let chartsConfig = [];
    ...
    _prepareInfluxQuery(queries, deploymentId, nodeId, nodeInstanceId, from, to, timeGroup) {
        return _.map(queries, queryParams => {
            const selectWhat = this._sanitizeQuery(queryParams.qSelect);
            ...
            if (!_.isEmpty(selectWhat) && !_.isEmpty(selectFrom)) {
                if (
                    (_.includes(selectFrom, '${deploymentId}') && _.isEmpty(deploymentId)) ||
                ...
                selectWhere = _.replace(
                    selectWhere,
                    '${timeFilter}',
                    `time > ${from} and time < ${to} group by time(${timeGroup})`
                );

                if (_.isEmpty(selectWhere)) return { qSelect: selectWhat, qFrom: selectFrom };
    ...
    _isEmptyResponse(widget, data) {
        return data.state === widget.definition.EMPTY_RESPONSE_STATE;
    },

    _isWidgetNotConfigured(widget, data) {
    ...
    fetchParams(widget, toolbox) {
        ...
        const nodeFilterFromWidget = widget.configuration.nodeFilter;
        ...
        const timeFilterFromWidget = widget.configuration.timeFilter;
        const timeFilterFromContext = toolbox.getContext().getValue('timeFilter');
        ...
        const timeResolution = _.get(timeFilterFromContext, 'resolution', timeFilterFromWidget.resolution);
        const timeUnit = _.get(timeFilterFromContext, 'unit', timeFilterFromWidget.unit);
        const timeGroup = `${timeResolution}${timeUnit}`;
    ...
    fetchData(widget, toolbox, params) {
        ...
        if (!_.isEmpty(preparedQuery)) {
            ...
            return actions
                .doRunQuery(preparedQuery.qSelect, preparedQuery.qFrom, preparedQuery.qWhere)
                .then(data => {
                    toolbox.loading(false);
                    const formattedResponse = _.map(data, metric => ({
                        name: _.last(_.split(metric.name, '.')),
                        points: metric.points
                    }));
                    return Promise.resolve(
                        _.isEmpty(data) ? { state: widget.definition.EMPTY_RESPONSE_STATE } : formattedResponse
                    );
                })
                .catch(error => {
                    toolbox.loading(false);
                    return Promise.reject(
                        `${'There was a problem while querying for data. ' +
                            'Please check your Influx query syntax and try again. Error: '}${error.message}` || error
                    );
                });
        }
        if (!_.isEmpty(deploymentId) && !_.isEmpty(nodeInstanceId) && !_.isEmpty(metrics)) {
            toolbox.loading(true);
            return actions
    ...
    render(widget, data, error, toolbox) {
        const { charts, query, type } = widget.configuration;
        const { Message, Icon } = Stage.Basic;

        if (_.isEmpty(data)) {
            return <Stage.Basic.Loading />;
        }
        if (this._isWidgetNotConfigured(widget, data)) {
            return (
                <Message info icon>
                    <Icon name="info" />
                    Please select deployment, node instance and metric in widget's configuration to present the data
                    graph.
                </Message>
            );
        }
        if (this._isEmptyResponse(widget, data)) {
            return (
                <Message info icon>
                    <Icon name="ban" />
