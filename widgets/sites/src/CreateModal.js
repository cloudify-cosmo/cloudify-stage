...
export default class CreateModal extends React.Component {
    ...
    _createSite() {
        ...
        actions.doCreate(this.state.siteName, this.state.siteVisibility, this.state.siteLocation).then(() => {
    ...
    render() {
        let {ApproveButton, Button, CancelButton, Icon, Form, Modal, VisibilityField} = Stage.Basic;
        const createButton = <Button content='Create' icon='add' labelPosition='left' />;

        return (
            <Modal trigger={createButton} open={this.state.open} onOpen={() => this.setState({open:true})}
                   onClose={() => this.setState({open:false})}>
                <Modal.Header>
                    <Icon name='add' /> Create site
                    <VisibilityField visibility={this.state.siteVisibility} className="rightFloated"
                ...
                <Modal.Content>
                    <Form loading={this.state.loading} errors={this.state.errors}
                        ...
                        <Form.Field label='Name' error={this.state.errors.siteName} required>
                            <Form.Input name='siteName' value={this.state.siteName}
                                        onChange={this._handleInputChange.bind(this)}/>
                        </Form.Field>
                        <Form.Field label='Location' error={this.state.errors.siteLocation}>
                            <Form.Input name='siteLocation' value={this.state.siteLocation}
                                        placeholder="latitude, longitude (32.166369, 34.810893)" onChange={this._handleInputChange.bind(this)}/>
                        </Form.Field>
                    </Form>
                </Modal.Content>
