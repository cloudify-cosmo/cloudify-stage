...
export default class SitesMap extends React.Component {
    ...
    shouldComponentUpdate(nextProps) {
        return !_.isEqual(this.props.showAllLabels, nextProps.showAllLabels)
    ...
    _openPopup(marker) {
        if (marker && marker.leafletElement) {
            window.setTimeout(() => {marker.leafletElement.openPopup()})
        }
    }
    ...
    _createMarkers() {
        ...
        _.forEach(this.props.data, (site) => {
            let iconUrl = widgetResourceUrl(widgetDir, markerIconPath(site.color), false);
            let icon = new L.Icon({
                iconUrl: iconUrl,
                shadowUrl: shadowUrl,
                iconSize: [25, 41],
            ...
            markers.push(
                <Marker position={[site.latitude, site.longitude]} ref={showLabels}
                        key={`siteMarker${site.name}`} riseOnHover={true} icon={icon}>
    ...
    render() {
        ...
        if (!this.props.isMapAvailable) {
            ...
            return (
                <MapMessage text={NO_INTERNET_MESSAGE}/>
            );
        }
        ...
        if (_.isEmpty(markers)) {
            return (
                <NoDataMessage sitesAreDefined={this.props.sitesAreDefined}/>
            );
        }
        ...
        return (
            <Map className='sites-map' center={mapOptions.position} zoom={mapOptions.zoom}
                 maxBounds={mapOptions.bounds} maxBoundsViscosity={mapOptions.viscosity}
                 minZoom={mapOptions.minZoom} maxZoom={mapOptions.maxZoom}>
...
function NoDataMessage({sitesAreDefined}) {
    ...
    return (
        <MapMessage text={NO_DATA_MESSAGE} addLink={true}/>
    );
}
    ...
    return (
        <MessageContainer wide={true} margin='30px auto'>
            <Message>
                ...
                {
                    addLink &&
                    <Link to={SITES_PAGE_PATH}>Site Management page.</Link>
                }
            </Message>
        </MessageContainer>
    );
