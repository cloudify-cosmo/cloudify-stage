...
Stage.defineWidget({
    ...
    fetchData: (widget, toolbox, params) => {
        return Promise.all([
            widget.configuration.filterByBlueprints
                ? toolbox.getManager().doGetFull('/blueprints?_include=id')
                : Promise.resolve({}),
            widget.configuration.filterByDeployments
                ? toolbox.getManager().doGetFull('/deployments?_include=id,blueprint_id,site_name')
                : Promise.resolve({}),
            widget.configuration.filterByNodes
                ? toolbox.getManager().doGetFull('/nodes?_include=id,blueprint_id,deployment_id')
                : Promise.resolve({}),
            widget.configuration.filterByNodeInstances
                ? toolbox.getManager().doGetFull('/node-instances?_include=id,deployment_id,node_id')
                : Promise.resolve({}),
            widget.configuration.filterByExecutions || widget.configuration.filterByExecutionsStatus
                ? toolbox
                      .getManager()
                      .doGetFull('/executions?_include=id,blueprint_id,deployment_id,workflow_id,status_display')
                : Promise.resolve({}),
            widget.configuration.filterBySiteName
                ? toolbox.getManager().doGetFull('/sites?_include=name')
                : Promise.resolve({})
        ]).then(results => {
            return {
    ...
    _processData(blueprintId, deploymentId, nodeId, nodeInstanceId, executionId, executionStatus, siteName, data) {
        ...
        processedData.allExecutions = { ...processedData.executions };
        if (!_.isNil(blueprintId)) {
            const blueprintIdArray = _.castArray(blueprintId);
            processedData.deployments.items = _.filter(processedData.deployments.items, deployment =>
                _.includes(blueprintIdArray, deployment.blueprint_id)
            );
            processedData.nodes.items = _.filter(processedData.nodes.items, node =>
                _.includes(blueprintIdArray, node.blueprint_id)
            );
            processedData.executions.items = _.filter(processedData.executions.items, execution =>
                _.includes(blueprintIdArray, execution.blueprint_id)
            );
        }
        ...
        if (!_.isNil(deploymentId)) {
            ...
            processedData.nodes.items = _.filter(processedData.nodes.items, node =>
                _.includes(deploymentIdArray, node.deployment_id)
            );
            processedData.nodeInstances.items = _.filter(processedData.nodeInstances.items, nodeInstance =>
                _.includes(deploymentIdArray, nodeInstance.deployment_id)
            );
            processedData.executions.items = _.filter(processedData.executions.items, execution =>
                _.includes(deploymentIdArray, execution.deployment_id)
            );
        }
        ...
        if (!_.isNil(nodeId)) {
            ...
            processedData.nodeInstances.items = _.filter(processedData.nodeInstances.items, nodeInstance =>
                _.includes(nodeIdArray, nodeInstance.node_id)
            );
        }

        processedData.executionStatuses = { ...processedData.executions };
        if (!_.isNil(executionId)) {
            const executionIdArray = _.castArray(executionId);
            processedData.executionStatuses.items = _.filter(processedData.executionStatuses.items, execution =>
                _.includes(executionIdArray, execution.id)
            );
        }
    ...
    render(widget, data, error, toolbox) {
        ...
        return <Filter configuration={widget.configuration} data={processedData} toolbox={toolbox} />;
    }
});
