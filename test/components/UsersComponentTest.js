...
describe('(Component) Users', () => {
    ...
    it('renders user menu with full options list ',()=>{
        ...
        expect(wrapper.find(Dropdown.Item).get(0).props.text).to.equal('Configure'); // Having Configure option
        expect(wrapper.find(Dropdown.Item).get(0).props.icon).to.equal('settings'); // Having settings icon for Configure option
