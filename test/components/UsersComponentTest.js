...
describe('(Component) Users', () => {
    ...
    it('renders user menu with full options list ',()=>{
        ...
        expect(wrapper.find(Dropdown.Item).length).to.equal(4); // 4 menu options
        ...
        expect(wrapper.find(Dropdown.Item).get(3).props.text).to.equal('Logout'); // Having Logout option
        expect(wrapper.find(Dropdown.Item).get(3).props.icon).to.equal('log out'); // Having log out icon for Logout option
    });
    ...
    it('renders user menu with full options list and edit mode selected ',()=>{
        ...
        expect(wrapper.find(Dropdown)).to.have.length(1); // Showing the users menu
        expect(wrapper.find(Dropdown.Item).length).to.equal(4); // 3 menu options
