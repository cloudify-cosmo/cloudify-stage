...
export const mockSecretCreation = (secretName: string) =>
    cy.interceptSp('PUT', `/secrets/${secretName}`, req => req.reply({}));

export const mockSecretUpdate = (secretName: string) =>
    cy.interceptSp('PATCH', `/secrets/${secretName}`, req => {
        req.reply({});
    });
