pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '4.1.1.1-build']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'cloudify-stage']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '9f6aca75-ebff-4045-9919-b8ec6b5ccf9d', url: 'https://github.com/cloudify-cosmo/cloudify-stage.git']]])
                dir('cloudify-stage') {
                    sh '''sudo npm install
        ...
        stage('Pack') {
            steps {
                ...
                sh '''. ${JENKINS_HOME}/jobs/credentials.sh > /dev/null 2>&1
                      curl -u $GITHUB_USERNAME:$GITHUB_PASSWORD https://raw.githubusercontent.com/cloudify-cosmo/cloudify-premium/4.1.1.1-build/packages-urls/common_build_env.sh -o ./common_build_env.sh
