...
const page = (state = {}, action) => {
    switch (action.type) {
        ...
        case types.ADD_DRILLDOWN_PAGE:
            var pageData = { ...state, widgets: widgets(state.widgets, action) };
        ...
        case types.RENAME_PAGE:
            return { ...state, name: action.name, id: action.newPageId };
        default:
            return state;
    }
};
...
const pages = (state = [], action) => {
    switch (action.type) {
        ...
        case types.REMOVE_WIDGET:
            return state.map(page => {
                if (page.id === action.pageId) {
                    return { ...page, widgets: widgets(page.widgets, action) };
                }
                return page;
            });
        ...
        case types.REORDER_PAGE:
            var { pageIndex } = action;
            var { newPageIndex } = action;
            var realPageIndex = 0;
            var realNewPageIndex = 0;
