...
router.post('/resources', (req, res) => {
    ...
    request.get({ url: zipUrl as string, encoding: null }, (err, zipRes, body) => {
        if (err) {
            ...
            if (zipRes) {
                ...
                res.status(statusCode).send({
                    message: `The URL is not accessible - Error ${zipRes.statusCode} ${STATUS_CODES[zipRes.statusCode]}`
                });
            } else {
                res.status(400).send({
